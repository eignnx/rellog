...
use crate::{
    ast::{partial_txt::PartialTxt, RcTm, Tm},
    data_structures::Var,
    rt::{
        self,
...
enum VarOrTxt<'a> {
    Txt(Option<&'a PartialTxt>),
    Var(&'a Var),
    Err(&'a Tm),
}

impl<'a> From<&'a RcTm> for VarOrTxt<'a> {
    fn from(value: &'a RcTm) -> Self {
        match value.as_ref() {
            Tm::Nil => Self::Txt(None),
            Tm::Txt(txt) => Self::Txt(Some(txt)),
            Tm::Var(var) => Self::Var(var),
            Tm::Sym(..)
            | Tm::Int(..)
            | Tm::Block(..)
            | Tm::Rel(..)
            | Tm::BinOp(..)
            | Tm::Cons(..) => Self::Err(value),
        }
    }
}
...
fn mode_check(
    rel: &RcTm,
    u: UnifierSet,
    prefix: Option<&PartialTxt>,
    suffix: Option<&PartialTxt>,
    compound: Option<&PartialTxt>,
) -> Box<dyn SolnStream> {
    todo!()
